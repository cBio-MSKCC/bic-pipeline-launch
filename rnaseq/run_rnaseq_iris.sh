...
# constants
bic_rnaseq="/usersoftware/test01/opt/common/bic/internal/bic-rnaseq/current"
bic_diff="/usersoftware/test01/opt/common/bic/internal/bic-differentialabundance/current"
profile="slurm"
DE_only=false
rsync_only=false
...
MODULEPATH=/etc/modulefiles:/usr/share/modulefiles:/admin/software/lmod/modulefiles
...
module load migration-testing/java/openjdk17.0.10
...
job_to_run="sbatch -J \"rnaseq_${dir_name}\" -n 4 --time=300:00:00 --mem 8G --workdir=${an_dir} -o ${an_dir}/rnaseq.log -e ${an_dir}/rnaseq.err \
--wrap=\"nextflow run $bic_rnaseq \
-resume \
-profile $profile \
-ansi-log false \
-c $bic_rnaseq/conf/bic.config \
-c $bic_rnaseq/conf/juno.config \
...
if [ $rsync_only == false ]; then
    ...
    for cfile in $(ls $an_dir/contrasts*.csv 2> /dev/null); do
        ...
        job_to_run="sbatch -J \"diff_${dir_name}_${cnum}\" -n 4 --time=300:00:00 --mem 8G --workdir=${an_dir} $de_job_hold -o ${an_dir}/diff.log -e ${an_dir}/diff.err \
...
job_to_run="sbatch -J \"finalize_${dir_name}\" $rsync_job_hold -u ${email} --mail-type=END,FAIL \
-n 1 --time=300:00:00 --mem 2G --workdir=${an_dir} -o ${an_dir}/rsync.log -e ${an_dir}/rsync.err \
--mail-user ${email} \
